...
- The binding belongs to the Tumour Delineation slice composite node's Label
  layer and to no other panel. The HS Cube panel keeps no label layer: its
  bands run along S, so a one-band label map placed in that stack is off the
  plane the operator scrolls. Assert the composite node, not only the node in
  the scene: every property of the ground-truth node can be right while it is
  bound to no view and the status text still claims a comparison is on screen
  (corrected 2026-09-18, see Review findings).
...
Amended 2026-09-18. ADR-0003 decision 3 as accepted said outputs are shown
"with nothing composited under or over them", which the ground-truth layer
added to this card the same day contradicts. The owner amended decision 3 in
place rather than opening a superseding ADR. `AGENTS.md` asks for a new ADR to
change an accepted decision; the owner instructed that no extra artefact be
created for this branch, so the amendment is written into ADR-0003's Status and
decision 3 and dated there. This is recorded so the departure is visible rather
than silent.
...
Extended 2026-09-18 by the project owner, for the review corrections in Review
findings. One file was added:

- `extensions/SLIAFlow/CMakeLists.txt` - the extension-level file, which
  declared a `SlicerOpenIGTLink` dependency that ADR-0003 decision 2 makes
  false. Only `extensions/SLIAFlow/SLIAFlow/CMakeLists.txt`, the module-level
  file, was on the list before.

Extended 2026-09-24 by the project owner. Two files were added:

- `docs/hardware/acquisition_app_and_hardware.md` - reference documentation
  for the installed acquisition application and its hardware.
- `scripts/development/inspect-uc1-bmp.py` - diagnostic report generator for
  checking UC1 BMP row padding and decode behavior.
...
| 14 | Select `knn.bmp`, then `gtMap` again; then `kmeans.bmp`, then `gtMap` again | Delineation output is one box, so choosing an output turns the overlay off and choosing `gtMap` brings it back over the output picked last. After each pair the labels sit over the output just chosen. `kmeans.bmp` is painted from cluster numbers, so its colours are not expected to agree with the ground truth; only `svm.bmp` and `knn.bmp` share the legend | Not yet performed |
...
Added 2026-09-18, for the review corrections in `## Review findings`. Steps 16
to 22 each name the wrong result as well as the right one, because each is there
to show that a specific defect is gone; a step that only confirms the happy path
would have passed before the fix too. Run `.\scripts\development\build-sliaflow.ps1`
first and confirm every module file is `ok`, then start
`build\SLIAFlow\SlicerWithSLIAFlow.exe` by double-clicking it.

Steps 19, 21 and 22b use Slicer's Python console (`View > Python Console`, or
Ctrl+3) to inject a failure that cannot be produced by hand. Each one says how
to undo it, and step 23 checks that nothing was left injected. These three have
automated tests already; the manual step is only to see what the operator is
told and that the module recovers.

| # | Action | Expected observation | Result |
| --- | --- | --- | --- |
| 16 | With `gtMap` selected from step 12, look at the HS Cube panel and scroll its slice slider through the bands | The cube is unchanged: bands in greyscale, no coloured labels anywhere on it, and scrolling still moves through the spectrum. The labels are on Tumour Delineation only. Wrong result: any coloured label pixels on the HS Cube panel, or the panel going blank at some band | Not yet performed |
| 17 | On Tumour Delineation, zoom in with the mouse wheel and pan with the middle button until one small region fills the panel. Note what is on screen. Now select `pca.bmp`, `svm.bmp`, `knn.bmp`, `kmeans.bmp`, `gtMap` and `imageRGB.bmp` in turn | The zoom and position stay exactly where you left them through all six selections; only the picture under them changes. Wrong result: the view snapping back to the whole image on any selection. Then press Capture, wait for the new result: this time the view does refit, framed for the new case | Not yet performed |
| 18 | Open `build\SLIAFlow\SLIAFlow.s4ext` in a text editor and read the `depends` line. Then confirm SLIAFlow is in the STRATUM category and its panel loads | The line reads `depends NA`, not `depends SlicerOpenIGTLink`. The module loads with no missing-dependency warning in the Python console at startup | Not yet performed |
| 19 | Press Start. In the Python console run:<br>`w = slicer.modules.sliaflow.widgetRepresentation().self()`<br>`def boom(frame): raise RuntimeError("injected display failure")`<br>`w.logic._frameCallback = boom` | Within a moment LiveView stops. Status reads `The camera frame could not be drawn, so LiveView was stopped. The camera itself is fine. Reload the module, or switch to the SLIAFlow layout and start LiveView again.` The Python console shows a traceback headed `SLIAFlow: the camera frame could not be displayed`. Wrong result: the status telling you to check the device connection, which is the camera message and would send you to the wrong place. Press Start again: LiveView recovers, which is what that message promises | Not yet performed |
| 20 | With LiveView running, in the Python console run `w.logic._cameraCapture.release()` | LiveView stops and status reads `The camera stopped providing valid frames. Check the device connection and whether another application is using the camera.` - the other message, for the other fault. Press Start again to recover | Not yet performed |
| 21 | Press Capture once and let it finish, so the case queue is filled. In the Python console run `print([c.name for c in w.logic.casePool._queue][:3])` and note the first name. In PowerShell rename that case's dark reference:<br>`Rename-Item "C:\stratum\input\bin\bin\<case>\darkReference.dat" "darkReference.dat.off"`<br>Press Capture | The HS Cube panel fills with that case's cube first - `raw.dat` is intact, so it is read normally - and then the run is refused before UC1 starts. Status reads `Failed on recorded case <case>: darkReference.dat is missing. Press Capture to try again.` LiveView resumes, Capture re-enables, and the previous result stays marked `PREVIOUS RESULT - not from the current capture`. Wrong result: UC1 running at all on a case missing a calibration file. Rename it back immediately:<br>`Rename-Item "C:\stratum\input\bin\bin\<case>\darkReference.dat.off" "darkReference.dat"` | Not yet performed |
| 22a | Confirm `build\uc1\UC1\.uc1-runner.lock` does not exist after the refused run of step 21 and after a successful run | The file is absent both times. A run that is refused before it starts, and a run that finishes, each leave the staged build unlocked | Not yet performed |
| 22b | Optional. In the Python console run:<br>`import importlib, os`<br>`uc1run = importlib.import_module("SLIAFlowLib.SLIAFlowUc1Run")`<br>`realWrite = os.write`<br>`os.write = lambda *a, **k: (_ for _ in ()).throw(OSError("injected"))`<br>Press Capture, then immediately restore with `os.write = realWrite` | Status reads `Failed on recorded case <case>: The UC1 build lock ...\.uc1-runner.lock could not be written: injected ...` and `build\uc1\UC1\.uc1-runner.lock` does **not** exist afterwards. Wrong result: the lock file being left behind, which would make every later Capture refuse against a holder that does not exist. Press Capture again: it starts normally | Not yet performed |
| 23 | In the Python console run:<br>`realCollect = uc1run.collectOutputs` (import it as in 22b if you skipped that step)<br>`def boom(*a, **k): raise PermissionError("injected: the output folder is denied")`<br>`uc1run.collectOutputs = boom`<br>Press Capture and wait for UC1 to finish | The run completes and is reported as failed: status reads `Failed on recorded case <case>: UC1 finished on recorded case <case>, but its outputs could not be checked: injected: the output folder is denied Press Capture to try again.` LiveView resumes and Capture becomes enabled again. Wrong result, and the whole point of the step: LiveView staying frozen and Capture staying greyed out with no message, needing a restart of Slicer. Restore with `uc1run.collectOutputs = realCollect` and press Capture: a normal result returns | Not yet performed |
| 24 | Close-out. Confirm `input\bin\bin\<case>\darkReference.dat` is back under its real name and its size matches the other cases' pattern; confirm `build\uc1\UC1\.uc1-runner.lock` is absent; restart `SlicerWithSLIAFlow.exe` so no console injection survives; open SLIAFlow, Start, Capture, and leave the module for Welcome | The renamed file is restored, no lock remains, a clean run succeeds on the restarted app, and leaving the module restores the previous layout | Not yet performed |
...
- **A stale lock blocks every run.** A Slicer killed from Task Manager during a
  ...
  message names the file. This stands: never taking over another holder's lock
  is the decision, and a dead parent PID does not establish that its child
  stopped. Distinguished on 2026-09-18 from a lock SLIAFlow created and never
  owned, which is now removed rather than left behind (Review findings, 6).
...
- ADR-0003 amended 2026-09-18: decision 3 admits the ground-truth label
  overlay, its Status records the amendment, and its Consequences and
  Validation are brought up to date with the HS Cube panel no longer being
  reserved.
...
### Code review and independent verification (2026-09-18)

A Slicer-skill review of the whole branch, and an independent verification of
that review against `810ea10`, were run at the owner's request. Both reports
are in `SLIAFLOW_CODE_REVIEW.md` at the repository root. The owner directed
that the High and Medium findings be fixed on this branch, that the Low ones be
left, and that no new task card be opened. Applied on 2026-09-18:

1. **High - the ground truth was never displayed.** The `gtMap` label-layer
   binding sat in `_showCube`, the HS Cube panel, while `_showResult` cleared
   its own label layer unconditionally. Choosing `gtMap` therefore changed
   nothing on screen anywhere, while the status line said the comparison was
   on screen. Moved into `_showResult`; the cube panel now clears its label
   layer with a comment saying why it can never hold one.
   `SLIAFlowWidget.py`.
2. **High - an output-validation failure could leave Capture busy forever.**
   `Uc1Run._onProcessFinished` handled only `Uc1RunError` around
   `collectOutputs`. An `OSError` from the filesystem escaped a Qt slot, which
   drops it, so the completion callback never ran: LiveView stayed frozen and
   Capture stayed disabled for the rest of the session. It is now caught,
   logged with its traceback, and reported as a failed run.
   `SLIAFlowUc1Run.py`.
3. **Medium - the result view refitted on every output change.** The condition
   included the background node ID, so switching `svm.bmp` to `knn.bmp`
   discarded the operator's pan and zoom, against the comment directly above
   it. The background is now bound unconditionally and only the fit is guarded,
   by the capture ID and by the panel having been empty - an empty panel is
   still refitted, because a rebuilt layout gives the view a default field of
   view. `SLIAFlowWidget.py`.
4. **Medium - stale `SlicerOpenIGTLink` dependency.** `EXTENSION_DEPENDS` named
   an extension nothing in the module uses and ADR-0003 decision 2 forbids.
   Set to `"NA"`, the Slicer template's value for no dependencies.
   `extensions/SLIAFlow/CMakeLists.txt`.
5. **Medium - queued cases were not revalidated before execution.** The pool
   describes a case when it refills, which can be several captures earlier, and
   `assertRunnable` did not re-read it. A queued case whose `darkReference.dat`
   was truncated to zero bytes still started UC1. `assertCaseUnchanged` now
   re-reads the folder at the top of `assertRunnable`, the last point before
   the lock is taken, and refuses the run rather than selecting a replacement:
   a silent replacement would stamp the result with a case the operator never
   saw chosen. `SLIAFlowCasePool.py`, `SLIAFlowUc1Run.py`.
6. **Medium - a lock whose PID could not be written was left unowned.**
   `acquireLock` created the file before writing it, and `_lockHeld` becomes
   true only once it returns, so a failed write left a lock nothing would ever
   release. The file this call created is now removed before refusing. Distinct
   from the deliberate stale-lock policy, which is unchanged.
   `SLIAFlowUc1Run.py`.
7. **Medium - a display failure was reported as a camera failure.** The camera
   read and the display callback shared one `except`, so a closed view or a
   removed node told the operator to check the device connection, which no
   amount of checking would fix. They are now separate blocks with separate
   messages, and both log their traceback. `SLIAFlowLogic.py`.

Also corrected, as documentation the fixes made wrong or that was already
stale: ADR-0003 (decision 3 amendment, HS Cube consequence, validation
bullets), `extensions/SLIAFlow/README.md` (the HS Cube panel is no longer
black), and the `DEFAULT_RESULT_OUTPUT` comment in `SLIAFlowParameterNode.py`,
which described a majority-voting map that is not among the five outputs.

Not applied, deliberately: the Low findings (11-22 of the review), which the
owner scoped out; the review's proposal to delete the parameter write in
`_onResultOutputChanged`, which the verification showed to be load-bearing
because the widget handler runs before the GUI connector updates the parameter;
and automatic PID or age-based takeover of a stale lock, which the verification
established is a policy change rather than a routine fix.

New tests, all of which fail against the code as it was before these fixes:

- `test_groundTruthReachesTheResultPanelLabelLayer` (headful) - the Tumour
  Delineation composite node's label layer carries the ground truth at the
  stated opacity while the chosen output stays on the background layer, and
  the layer is cleared when an output is chosen instead.
- `test_cubePanelStaysAloneWhenGroundTruthIsSelected` (headful) - the HS Cube
  panel carries no label layer even with `gtMap` selected.
- `test_switchingOutputWithinOneResultKeepsTheFraming` (headful) - a zoom
  survives stepping through `svm.bmp`, `knn.bmp`, `gtMap` and `pca.bmp`.
- `test_caseChangedOnDiskIsRefusedRatherThanRunOn` - a case that changed size,
  and one that no longer loads, are both refused before the process starts and
  leave no lock behind.
- `test_lockThatCannotBeWrittenIsNotLeftBehind` - a failed PID write removes
  the lock and the build stays usable.
- `test_unexpectedValidationFailureStillReportsTheRun` - a `PermissionError`
  from the output checks is still reported through the completion callback,
  with the lock released.

Validation: `run-slicer-tests.ps1 -Target Source` and the same with `-Headful`,
both 72 tests, exit 0, 9 and 1 skipped respectively (the skips are the
headful-only tests under the headless target, and the headless-only fallback
test under the headful one).

Still needed: these changes have not been seen in the built application.
Findings 1, 3 and 7 change what the operator sees and need manual verification
per `.ai/workflows/manual-verification-workflow.md` - in particular a capture
on a case with tumour pixels, choosing `gtMap`, and confirming the labels are
drawn over the output with the Label layer's opacity slider working on them.
